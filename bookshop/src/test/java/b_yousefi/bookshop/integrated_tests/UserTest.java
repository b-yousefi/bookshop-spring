...
/**
 * Created by: b.yousefi Date: 5/17/2020
 */
@Sql("create_user.sql")
public class UserTest extends IntegratedTest {
        ...
        void setUp() {
                ...
                this.userList.add(User.builder().id(2L).username("user_test1").password("user_test1").role("ROLE_USER")
                                .build());
        }

        @Test
        void register() throws Exception {
                getMVC().perform(post("/register").contentType(MediaType.APPLICATION_JSON)
                                .content("{ \"username\" : \"b_yousefi\"" + ", \"password\" : \"b_yousefi\" "
                                                + ", \"firstName\" : \"Behnaz\" " + ", \"lastName\" : \"Yousefi\" "
                ...
                getMVC().perform(get(getPathTo(USERS_PATH_NAME) + 2 + "/orders").header("Authorization", getUserToken())
                                .with(user(getUser().getUsername()).password(getUser().getPassword()).roles("USER")))
                                .andDo(print()).andExpect(jsonPath("$._embedded.orders", hasSize(1)));
        }
        ...
        void authenticate() throws Exception {
                ...
                getMVC().perform(post("/authenticate").contentType(MediaType.APPLICATION_JSON)
                                .content("{ \"username\" : \"b_yousefi\"" + ", \"password\" : \"b_yousefi\" }"))
        ...
        void authenticate_with_wrong_password_gets_Unauthorized() throws Exception {
                getMVC().perform(post("/register").contentType(MediaType.APPLICATION_JSON)
                                .content("{ \"username\" : \"b_yousefi\"" + ", \"password\" : \"b_yousefi2\" "
                                                + ", \"firstName\" : \"Behnaz\" " + ", \"lastName\" : \"Yousefi\" "
                                                + ", \"email\" : \"b.yousefi2912@gmail.com\" " + "}"))
                                .andExpect(status().isCreated());
        ...
        void when_user_is_anonymous_then_get_is_FORBIDDEN() throws Exception {
                ...
                getMVC().perform(get(getPathTo(USERS_PATH_NAME))).andExpect(status().isUnauthorized());
        }
        ...
        void when_user_with_role_USER_then_can_get_its_own_data() throws Exception {
                ...
                getMVC().perform(get(getPathTo(USERS_PATH_NAME) + 2).header("Authorization", getUserToken())
                                .with(user(getUser().getUsername()).password(getUser().getPassword()).roles("USER")))
                                .andExpect(jsonPath("$.username").value(getUser().getUsername()))
                                .andExpect(jsonPath("$._links.self.href", endsWith("2")));

                // findUser
                getMVC().perform(get(getSearchPathTo(USERS_PATH_NAME) + "findUser").param("username", "user_test1")
                                .header("Authorization", getUserToken())
                                .with(user(getUser().getUsername()).password(getUser().getPassword()).roles("USER")))
                                .andExpect(jsonPath("$.username").value(getUser().getUsername()))
                                .andExpect(jsonPath("$._links.self.href", endsWith("2")));
                ...
                getMVC().perform(get(getPathTo(USERS_PATH_NAME) + 1).header("Authorization", getUserToken())
                                .with(user(getUser().getUsername()).password(getUser().getPassword()).roles("USER")))
                                .andExpect(status().isForbidden());

                // findUser
                getMVC().perform(get(getSearchPathTo(USERS_PATH_NAME) + "findUser").param("username", "admin")
                                .header("Authorization", getUserToken())
                                .with(user(getUser().getUsername()).password(getUser().getPassword()).roles("USER")))
                                .andExpect(status().isForbidden());
        }
        ...
        void post_from_users_path_is_only_for_users_with_role_ADMIN() throws Exception {
                ...
                getMVC().perform(post(getPathTo(USERS_PATH_NAME))).andExpect(status().isUnauthorized());

                // users with USER role cannot post to users path
                getMVC().perform(post(getPathTo(USERS_PATH_NAME)).contentType(MediaType.APPLICATION_JSON)
                                .content("{" + "  \"username\": \"username1\"," + "  \"password\": \"username1\","
        ...
        void when_user_is_anonymous_then_patch_is_FORBIDDEN() throws Exception {
                // check there are two users the username of the second one is "user_test1" with
                // id =2
                getMVC().perform(get(getPathTo(USERS_PATH_NAME)).header("Authorization", getAdminToken())
                                .with(user(getAdmin().getUsername()).password(getAdmin().getPassword()).roles("ADMIN")))
                                .andExpect(jsonPath(JSON_PATH_TO_LIST, hasSize(2)))
                ...
                getMVC().perform(patch(getPathTo(USERS_PATH_NAME))).andExpect(status().isUnauthorized());
        }

        @Test
        void when_user_with_role_USER_then_can_patch_its_own_data() throws Exception {
                // check there is a user with username "user_test1" with id =2 and phone number
                // = 989352229977
                getMVC().perform(get(getPathTo(USERS_PATH_NAME) + 2).header("Authorization", getUserToken())
                                .with(user(getUser().getUsername()).password(getUser().getPassword()).roles("USER")))
                ...
                getMVC().perform(patch(getPathTo(USERS_PATH_NAME) + 1).contentType(MediaType.APPLICATION_JSON)
                                .content("{" + "  \"username\": \"username1\"," + "  \"password\": \"username1\","
                ...
                getMVC().perform(patch(getPathTo(USERS_PATH_NAME) + 2).contentType(MediaType.APPLICATION_JSON)
                                .content("{" + "  \"username\": \"user_test1\"," + "  \"password\": \"user_test1\","
                ...
                getMVC().perform(get(getPathTo(USERS_PATH_NAME) + 2).header("Authorization", getUserToken())
                                .with(user(getUser().getUsername()).password(getUser().getPassword()).roles("USER")))
                                .andDo(print()).andExpect(jsonPath("$.username").value("user_test1"))
        ...
        void when_user_with_role_ADMIN_then_can_patch_any_users() throws Exception {
                ...
                getMVC().perform(get(getPathTo(USERS_PATH_NAME) + 2).header("Authorization", getAdminToken())
                                .with(user(getAdmin().getUsername()).password(getAdmin().getPassword()).roles("ADMIN")))
                                .andExpect(jsonPath("$.username").value("user_test1"))
                                .andExpect(jsonPath("$._links.self.href", endsWith("2")));

                // users with ADMIN role can patch to users path and change the username and
                // password to "username1"
                getMVC().perform(patch(getPathTo(USERS_PATH_NAME) + 2).contentType(MediaType.APPLICATION_JSON)
                                .content("{" + "  \"username\": \"username1\"," + "  \"password\": \"username1\","
        ...
        void put_from_users_path_is_only_for_users_with_role_ADMIN() throws Exception {
                ...
                getMVC().perform(put(getPathTo(USERS_PATH_NAME))).andExpect(status().isUnauthorized());
                ...
                getMVC().perform(put(getPathTo(USERS_PATH_NAME)).header("Authorization", getUserToken())
                                .with(user(getUser().getUsername()).password(getUser().getPassword()).roles("USER")))
                                .andExpect(status().isForbidden());
                // users with ADMIN role can put to users path and change the username and
                // password to "username1"
                getMVC().perform(put(getPathTo(USERS_PATH_NAME) + 2).contentType(MediaType.APPLICATION_JSON)
                                .content("{" + "  \"username\": \"username1\"," + "  \"password\": \"username1\","
                                                + "  \"phoneNumber\": \"989352228877\""
                                                + ", \"firstName\" : \"Behnaz\" " + ", \"lastName\" : \"Yousefi\" "
                                                + ", \"email\" : \"b.yousefi2912@gmail.com\" " + "}")
        ...
        void delete_from_users_path_is_only_for_users_with_role_ADMIN() throws Exception {
                ...
                getMVC().perform(delete(getPathTo(USERS_PATH_NAME) + 2)).andExpect(status().isUnauthorized());
